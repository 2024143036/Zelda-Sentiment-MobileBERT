...
### 📱 ② 구글 플레이 스토어 (Google Play Store): 모바일 컴패니언 생태계를 통한 라이트 게이머의 정서 인양
- **기획적 명분 및 필요성**: 《젤다의 전설》 시리즈는 닌텐도 콘솔 독점작이므로 구글 플레이 마켓에 게임 자체가 존재하지는 않습니다. 그러나 전 세계 수백만 명의 젤다 게이머들은 플레이 편의성을 위해 구글 마켓에서 《닌텐도 스위치 온라인 공식 앱》 및 다양한 《비공식 하이랄 지도/공략 컴패니언 가이드 앱》들을 필수적으로 다운로드하여 활용합니다. 본 연구는 이 앱들의 리뷰 섹션에 적힌 15,000건의 대규모 텍스트를 인양하였습니다. 이는 전문 비평가(메타크리틱)의 시선에서 벗어나, 모바일 환경에 익숙한 일반 대중(라이트 게이머)들이 모바일 컴패니언 앱을 이용하며 털어놓은 조작감, 무기 내구도 스트레스, 하드웨어 최적화에 대한 가장 직관적이고 솔직한 '날것의 유저 정서'를 확보할 수 있는 독보적인 대조군 채널입니다.
- **데이터 정형화 및 수집 기법**: 구글 플레이 백엔드 API 세션의 동적 패킷 흐름을 안전하게 링킹하는 전용 파서 엔진을 가동하였습니다. 구글의 강력한 실시간 트래픽 제어(Rate Limiting) 레이어 속에서 세션 끊김과 데이터 오염을 방지하기 위해 호출 단위별로 데이터 무결성 검증 로직을 매핑하였으며, 타겟 타이틀(야숨, 왕눈, 스소 리마스터 버전) 생태계에 대응하는 총 15,000건의 고순도 코퍼스를 무결하게 정형화하여 인양하는 데 성공하였습니다.
