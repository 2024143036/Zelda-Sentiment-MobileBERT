...
### 📱 ② 구글 플레이 스토어(Google Play Store): 모바일 컴패니언 리뷰 내 게임 평 정서 추출
- **기획적 명분 및 필요성**: 《젤다의 전설》 시리즈는 닌텐도 플랫폼 독점작이므로 구글 플레이 마켓에 게임 자체가 존재하지는 않습니다. 그러나 전 세계 수백만 명의 유저들은 플레이 편의성을 위해 구글 마켓에서 《닌텐도 스위치 온라인 앱》 및 다양한 《비공식 하이랄 지도/공략 컴패니언 앱》들을 필수로 활용합니다. 본 연구는 이 앱들의 사용자 리뷰란에서 단순한 앱 자체 불만을 제외하고, **유저들이 텍스트 내에서 언급한 '실제 젤다 시리즈(야숨·왕눈·스소)에 대한 정성적 비평 및 평가' 데이터만 정밀하게 추려내어(Filtering) 15,000건의 코퍼스를 인양**하였습니다. 이를 통해 전문 비평가(메타크리틱)의 시선에서 벗어나, 대중 게이머들이 컴패니언 앱을 이용하며 털어놓은 게임 기믹 스트레스와 하드웨어 최적화에 대한 가장 날것의 직관적인 대조군 정서를 확보하였습니다.
- **데이터 정형화 및 수집 기법**: 구글 플레이 백엔드 API 세션을 안전하게 링킹하는 전용 파서 엔진을 가동하였습니다. 강력한 실시간 트래픽 제어(Rate Limiting) 레이어 속에서 세션 끊김을 방지하기 위해 무결성 검증 로직을 매핑하였으며, 텍스트 내 주요 타이틀 키워드 매칭 파이프라인을 통해 정형화된 고순도 데이터셋을 성공적으로 구축하였습니다.
